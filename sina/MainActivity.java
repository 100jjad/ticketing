...
//                    imageView10.getLayoutParams().width = (int) getResources().getDimension(R.dimen.small_size);
        ...
        news.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Pair[] pair = new Pair[1];
                pair[0] = new Pair<View, String>(imageView16, "logo_news");
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    ActivityOptions options = ActivityOptions.makeSceneTransitionAnimation(MainActivity.this,
                            pair);
                    flag_first = true;
        ...
        news.setOnLongClickListener(new View.OnLongClickListener()
        {
            ...
            {
                Intent intent = new Intent(MainActivity.this , Advanace_Color_Change.class);
...
//
    ...
    public void fake_list()
    {

        Product_Scroll_Model entry = new Product_Scroll_Model("صفحه اول" , 1 ,0 , 1);
        temp.add(entry);
        entry = new Product_Scroll_Model("صفحه دوم" , 2 ,0 , 2);
        temp.add(entry);
        entry = new Product_Scroll_Model("صفحه سوم" , 3 ,4 , 3);
        temp.add(entry);
        entry = new Product_Scroll_Model("صفحه چهارم" , 4 ,5 , 4);
        temp.add(entry);
        entry = new Product_Scroll_Model("صفحه پنجم" , 5 ,0 , 5);
        temp.add(entry);
        entry = new Product_Scroll_Model("صفحه ششم" , 6 ,2 , 6);
        temp.add(entry);
        entry = new Product_Scroll_Model("صفحه هفتم" , 7 ,0 , 7);
        temp.add(entry);
        entry = new Product_Scroll_Model("صفحه هشتم" , 8 ,5 , 8);
        temp.add(entry);
        entry = new Product_Scroll_Model("صفحه نهم" , 9 ,3 , 9);
        temp.add(entry);
        entry = new Product_Scroll_Model("صفحه دهم" , 10 ,1 , 10);
        temp.add(entry);
    }
}
